...
export class LocationsController {
  ...
  @ApiOperation({ summary: 'Submit a new location booking request for a production' })
  @ApiResponse({ status: 201, description: 'Location booking request submitted.' })
  @ApiResponse({ status: 422, description: 'Double-booking date conflict or invalid date range.' })
  create(@Body() createDto: CreateLocationDto) {
    return this.locationsService.create(createDto);
  }

  @Get()
  @ApiOperation({ summary: 'List all scheduled location bookings for a production' })
  ...
  @ApiOperation({ summary: 'Update location booking status with Mongoose transaction & overlap conflict checks' })
  @ApiResponse({ status: 200, description: 'Location booking status updated successfully.' })
  @ApiResponse({ status: 422, description: 'Date collision conflict with an existing booked location.' })
  updateStatus(
    @Param('id') id: string,
