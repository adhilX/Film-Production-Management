...
export const fundsService = {
  async getBudget(productionId: string): Promise<Budget> {
    const res = await axiosClient.get<Budget>(`/productions/${productionId}/funds/budget`);
    return res.data;
  },
  ...
  async updateBudget(
    ...
    const res = await axiosClient.patch<Budget>(
  ...
  async getFundRequests(productionId: string): Promise<FundRequest[]> {
    const res = await axiosClient.get<FundRequest[]>(
  ...
  async createFundRequest(
    ...
    const res = await axiosClient.post<FundRequest>(
      `/productions/${productionId}/funds/requests`,
      payload,
    );
    return res.data;
  ...
  async updateFundRequest(
    ...
    const res = await axiosClient.patch<FundRequest>(
      `/productions/${productionId}/funds/requests/${requestId}`,
      payload,
    );
    return res.data;
