...
export const costumesService = {
  async getCostumes(
    ...
    const res = await axiosClient.get<Costume[]>(`/productions/${productionId}/costumes`, { params });
    return res.data;
  },

  async getCostume(productionId: string, costumeId: string): Promise<Costume> {
    const res = await axiosClient.get<Costume>(`/productions/${productionId}/costumes/${costumeId}`);
    return res.data;
  },

  async createCostume(productionId: string, payload: Partial<Costume>): Promise<Costume> {
    const res = await axiosClient.post<Costume>(`/productions/${productionId}/costumes`, payload);
    return res.data;
  },

  async updateCostume(productionId: string, costumeId: string, payload: Partial<Costume>): Promise<Costume> {
    const res = await axiosClient.patch<Costume>(`/productions/${productionId}/costumes/${costumeId}`, payload);
    return res.data;
  },

  async deleteCostume(productionId: string, costumeId: string): Promise<void> {
    await axiosClient.delete(`/productions/${productionId}/costumes/${costumeId}`);
  },

  async getAssignments(productionId: string): Promise<CostumeAssignment[]> {
    const res = await axiosClient.get<CostumeAssignment[]>(`/productions/${productionId}/costumes/assignments`);
    return res.data;
  },
  ...
  async assignCostume(
    ...
    const res = await axiosClient.post<CostumeAssignment>(
  ...
  async returnCostume(
    ...
    const res = await axiosClient.patch<CostumeAssignment>(
