...
export default function EditRequestModal({
  ...
  const {
    ...
    defaultValues: {
      ...
      requestedAmount: request.requestedAmount / 100,
    },
  });
  ...
  const onSubmit = async (values: RequestFormValues) => {
    ...
    try {
      ...
      await fundsService.updateFundRequest(selectedProduction._id, request._id, {
        title: values.title,
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
      <div className="bg-white border border-slate-200 rounded-2xl max-w-lg w-full p-6 shadow-xl space-y-5 animate-in zoom-in-95 duration-200">
        <div>
          <h3 className="text-base font-bold text-slate-900">Edit Fund Request</h3>
          <p className="text-[11px] text-slate-500 mt-1">
            Update pending fund request details.
          </p>
        </div>
